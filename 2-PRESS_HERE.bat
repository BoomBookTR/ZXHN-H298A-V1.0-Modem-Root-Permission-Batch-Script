...
:export
python examples/decode.py --serial %SeriNo% config.bin config.xml | find /i "Successfully decoded using serial" & if not errorlevel 1 (
echo *************************************************
goto :exportbitti
) else (
echo *************************************************
...
python examples/encode.py --serial %SeriNo% --signature "ZXHN H298A V1.0" config.xml config.bin | find /i "Done" & if not errorlevel 1 (
echo *************************************************
goto :importbitti
) else (
echo *************************************************
